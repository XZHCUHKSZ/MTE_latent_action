...
def evaluate(policy,seeds,anchor=None,agents=4,progress=None):
    ...
    for seed in seeds:
        env=make_env(int(seed),agents,23);rng=np.random.default_rng(int(seed)+901)
        ...
        try:
            ...
            if progress:progress(episode=len(returns),episodes=len(seeds))
