"""Agent-count dimension adapter; original recurrent class and objective."""
import numpy as np
import torch
...
def train_history_policy(positions, z, train_count, seed, updates, folder, progress, device="cuda"):
    """Stage-isolated orchestration, reuses original GRU; never receives actions."""
    seed_all(seed)
    folder.mkdir()
    e, t, d = z.shape
    od=positions.shape[-1]
    assert od in (16,24,32) and t==22 and positions.shape==(e,26,od)
    ...
    policy = RecurrentPolicy(od, d).to(device)
