...
视觉低标签补证：`experiments/visual_low_label.py` 复用原五个冻结视觉前端与history模型，扩展B1/B2/B4、核验后复用B8。正式低预算派发前要求旧预算权重与闭环逐步重放通过；不重训前端、不改变原四智能体任务。见[实验问题、预算和准入检查](docs/visual_low_label.md)。

全清单补测：`experiments/mpe_inventory_completion.py` 对齐原论文29个配置标签与四档无标签数据量，包含此前遗漏的Tree规模曲线、Continuous、IDM等，详见[全清单协议](docs/mpe_full_inventory.md)。复用和新增单元分别计数，已完成的旧结果不覆盖。

2026-09-18 MPE时间修正后的补齐入口：`experiments/mpe_evidence_completion.py`。补测原始Edge-CARA/Tree、Simple-raw、固定标签的无标签数据规模及冻结动作/交互/donor诊断，见[协议与信息边界](docs/mpe_evidence_completion.md)。旧结果只读保留；新增结果在独立outputs目录，未自动替换论文或上传GitHub。

新增审查诊断：`experiments/target_effects.py` 分别检查已知 slot、真实动作效应和 history 迁移，详见 [实验说明](docs/target_effects_experiments.md)。这是冻结模型的评估扩展；原正式训练入口、结果和通过冒烟测试的交付快照保持不变。
